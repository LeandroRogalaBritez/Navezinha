...
@onready var lb_level = $lbLevel
@onready var lb_xp = $lbXP
@onready var lb_points = $lbPoints
@onready var lb_strength = $lbStrength
@onready var lb_speed = $lbSpeed
@onready var lb_defense = $lbDefense
@onready var btn_strength = $btnStrength
@onready var btn_speed = $btnSpeed
@onready var btn_defense = $btnDefense
...
func _ready() -> void:
    ...
    btn_strength.pressed.connect(_on_strength_pressed)
...
func start_phase(phase):
    ship.reset_phase_progress()
    update_progress_ui(ship.level, ship.current_xp, ship.xp_to_next, ship.stat_points)
    update_stats_ui(ship.strength_points, ship.speed_points, ship.defense_points)
...
func update_progress_ui(level, xp, xp_to_next, points):
    lb_level.text = "Nível: " + str(level)
    lb_xp.text = "XP: " + str(xp) + "/" + str(xp_to_next)
    lb_points.text = "Pontos: " + str(points)

func update_stats_ui(strength, speed, defense):
    lb_strength.text = "Força: " + str(strength)
    lb_speed.text = "Velocidade: " + str(speed)
    lb_defense.text = "Defesa: " + str(defense)
...
func _on_strength_pressed():
    ship.allocate_point("strength")

func _on_speed_pressed():
    ship.allocate_point("speed")

func _on_defense_pressed():
    ship.allocate_point("defense")
